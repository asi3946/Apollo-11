...
# ファイル名　月面着陸誘導方程式
# 著作権:    パブリックドメイン。
# ファイル名:    LUNAR_LANDING_GUIDANCE_EQUATIONS.agc
# 目的:     Luminary 1A ビルド 099 のソースコードの一部。
#        これは、アポロ11号の月着陸船（LM）用
#        アポロ誘導コンピュータ（AGC）のソースコードの一部である。
# アセンブラ:    yaYUL
# 連絡先:    HARTMUTH GUTSCHE <hgutsche@xplornet.com>.
# ウェブサイト:    www.ibiblio.org/apollo.
# ページ:    798-828
# 変更履歴:    2009-05-23 HG    ページ画像から転記。
#        2009-06-05 RSB    GAPでは有効だがyaYULでは無効だった
#                奇妙な不具合を修正。Luminary 131から
#                引き継がれるべきではなかった数行のコードを
#                削除。
#        2009-06-07 RSB    誤字を修正。
#
# このソースコードは、MIT博物館所蔵の紙媒体の
# デジタル画像から転記または適宜改変したものです。
# デジタル化はPaul Fjeld氏によって行われ、
# 博物館のDeborah Douglas氏が手配しました。 両名に深く感謝します。画像（保存容量の削減に伴い、
# 画質も低下しています）は、
# www.ibiblio.org/apollo でオンラインで閲覧可能です。何らかの理由で
# 画像が判読できない場合は、info@sandroid.org までご連絡ください。
# ポールが実際に作成した（はるかに）高画質の画像へのアクセスについて
# ご案内いたします。
#
# 紙媒体の文書には、一部次のように記載されています：
#
#    NASA 2021112-061 による AGC プログラム LMY99 のリビジョン 001 を組み立てる
#    1969年7月14日 16:27
...
# フライトシーケンステーブルは機能別に整理されています。 これらは、レジスタ WCHPHASE をインデックスとして参照されます：
#    WCHPHASE = -1 ---> IGNALG 点火アルゴリズム　
#    WCHPHASE =  0 ---> BRAKQUAD  ブレーキ誘導
#    WCHPHASE =  1 ---> APPRQUAD アプローチ降下フェーズ
#    WCHPHASE =  2 ---> VERTICAL 垂直降下フェーズ
